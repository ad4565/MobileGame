...
Shader "Shader Forge/Mobile_Vtx_Cutout_Wind" {
    Properties {
        ...
        _NoiseAmnt ("NoiseAmnt", Float ) = 0.4
        _NoiseSpeed ("NoiseSpeed", Float ) = 2
        _WindNoiseFrequency ("WindNoiseFrequency", Float ) = 50
        _Spec_Color ("Spec_Color", Color) = (0.5,0.5,0.5,1)
        _GlossAmnt ("GlossAmnt", Range(0.1, 1)) = 0.1
    ...
    SubShader {
        ...
        Pass {
            ...
            uniform float _WindNoiseFrequency;
            uniform float _NoiseAmnt;
            uniform float _NoiseSpeed;
            uniform float4 _Spec_Color;
            uniform float _GlossAmnt;
            struct VertexInput {
            ...
            VertexOutput vert (VertexInput v) {
                ...
                float node_8955 = 1.0;
                float2 node_9583 = floor(o.uv0 * _WindNoiseFrequency) / (_WindNoiseFrequency - 1);
                float2 node_6995_skew = node_9583 + 0.2127+node_9583.x*0.3713*node_9583.y;
                float2 node_6995_rnd = 4.789*sin(489.123*(node_6995_skew));
                float node_6995 = frac(node_6995_rnd.x*node_6995_rnd.y*(1+node_6995_skew.x));
                float4 node_6488 = _Time;
                float3 node_4488 = (sin((node_6995*(node_6488.g*_NoiseSpeed)))*v.normal*_NoiseAmnt);
                v.vertex.xyz += (node_8955*node_4488*(1.0 - o.vertexColor.a));
...
/// Final Color:
        ...
        Pass {
            ...
            float4 frag(VertexOutput i) : SV_Target {
                ...
                float roughness = 1.0 - _GlossAmnt;
